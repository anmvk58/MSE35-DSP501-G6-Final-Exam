...
class AECLoopbackApp(QWidget):
    def __init__(self):
        ...
        self.NLMS_FILTER_LENGTH = 512
        # MU: Kích thước bước (step size) - quyết định tốc độ hội tụ (trade-off: tốc độ vs ổn định)
        self.NLMS_MU = 0.01
    ...
    def init_ui(self):
        ...
        title_label = QLabel("Acoustic Echo Cancellation (AEC) Demo - NLMS Implemented")
        ...
        info_label = QLabel(
            ...
            "4. Bật <b>Bật AEC (NLMS)</b> để thấy sự khác biệt (quá trình hội tụ cần vài giây)."
        )
    ...
    def toggle_aec(self, state):
        ...
        self.status_label.setText(
            f"Trạng thái: Đang chạy | AEC: {'BẬT (NLMS)' if self.aec_enabled else 'TẮT'}"
        )

    def aec_process(self, near_end_frame: np.ndarray, far_end_frame: np.ndarray) -> np.ndarray:
        """
        *** TRIỂN KHAI THUẬT TOÁN AEC BẰNG NLMS (Normalized Least Mean Squares) ***

        Hàm này thực hiện Khử Tiếng Vọng bằng bộ lọc thích ứng.
        - near_end_frame: Tín hiệu từ Microphone (tín hiệu cận - gần).
        - far_end_frame: Tín hiệu đã phát ra loa (tín hiệu xa - dùng để ước lượng tiếng vọng).
        """
        ...
        for i in range(len(near_end_frame)):
            ...
            # 4. Cập nhật Trọng số (W) theo NLMS
